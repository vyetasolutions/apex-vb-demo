          ...
          return (
            <motion.button
              ...
              className={`flex h-28 w-28 flex-col items-center justify-center rounded-2xl shadow-neon transition ${
                open ? "bg-apex-aqua text-apex-void" : "bg-apex-panelLight"
              } ${dimmed ? "opacity-40" : ""}`}
            >
              {open ? (
                <>
                  <span className="font-display text-xl font-extrabold">+{r.points}</span>
                  <span className="text-[10px]">{r.bonus ? "bonus!" : "points"}</span>
                </>
              ) : (
